...
{ 
  ...
  dist = r.range;
   for (unsigned int p=(pixel_count/4); p<=3*(pixel_count/4); p++){
 	  unsigned int q = p;
 	  if( r.pixels[q].range < dist && r.pixels[q].robot->color[1] == 180)
 	  {
 			  closest_green = (int)q;
 			  dist = r.pixels[q].range;
 	  }
  ...
  for (unsigned int p=0; p<pixel_count; p++){
	  ...
	  green_robots_inrange += r.pixels[p].other_robots[1];
...
//  }
  ...
  switch(decision)
  {
  	  ...
  	  case 1: if(closest_red > -1){
  	  	  	  ...
  	  	  	  break;
  	  case 2: if(closest_green > -1){
  		  	  	  Uni::Robot* other = r.pixels[closest_green].robot;
  		  	  	  FollowRobot(r, other);
  	  	  	  }
  	  	  	  break;
	  case 3: if(closest_blue > -1){
...
//      }
      ...
      r->speed[0] = r->speed_max;
      r->speed[1] = 0.0;     // no turning. we may change this
    }
